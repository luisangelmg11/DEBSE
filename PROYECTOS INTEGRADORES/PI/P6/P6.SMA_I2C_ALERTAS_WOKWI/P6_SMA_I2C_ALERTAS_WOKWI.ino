...
//  Sensor presión/temp : BMP180 (Adafruit_BMP085_Unified)
...
// ── LIBRERÍAS ─────────────────────────────────────────────
#include <Wire.h>               // Comunicación I2C (OLED + BMP180)
#include <DHT.h>                // Sensor de temperatura y humedad DHT22
#include <Adafruit_BMP085_U.h>  // Sensor BMP180 (presión y temperatura)
...
float t_bmp   = 0;    // Temperatura leída por el BMP180 (°C)
...
//  - BMP180 : presión barométrica (hPa→Pa) y temperatura
...
void leerSensores() {
  ...
  bmp.getEvent(&event);
  if (event.pressure) {
    pres  = event.pressure * 100.0;  // Convierte hPa → Pa
...
//    Pantalla 1 — Temperaturas DHT22 y BMP180 + umbral configurable
  ...
  if (cfg) {
    ...
    switch (pantalla) {
      ...
      case 1:  // Temperatura
        ...
        oledRow(34, F("BMP180:"), t_bmp,  1, F(" C"));
